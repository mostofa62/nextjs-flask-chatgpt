...
import DropdownDefault from '@/app/components/DropdownDefault';
import { object } from "yup";
import PromptList from "@/app/components/message/PromptList";
...
const PdfUpload=()=> {
  ...
  const chatMessageonChange = (e: React.FormEvent<HTMLInputElement>): void => {
      setChatMessages(e.currentTarget.value);
  };

  const chatMessageSubmit = async(e:SyntheticEvent)=>{
    e.preventDefault();
    //console.log(chatMessages);

    const user_index = conversations.length;
    //console.log(user_index)
    const message_json = {
      message:chatMessages,
      user:authCtx.userId,
      username:authCtx.displayName,
      msgid:authCtx.activeMessageId,
      index:user_index
    };
    ...
    //const message_array = conversations.splice(user_index,0,message_json);
    //const message_array = conversations;
    ...
    if(data.user_message && data.bot_message){
      setConversations([
        ...conversations,
        data.user_message,
        data.bot_message
          ...
          <div className='flex h-full flex-col border-l border-stroke dark:border-strokedark xl:w-full'>
            ...
            <div className='no-scrollbar max-h-full space-y-3.5 overflow-auto px-6 py-7.5'>
            {
              ...
              <div className='max-w-125'>
                ...
                <p className='text-right text-xs'>{converse.datetime}</p>
              </div>
              }
              </div>
                )
            ...
            <div className='sticky bottom-0 border-t border-stroke bg-white py-5 px-6 dark:border-strokedark dark:bg-boxdark'>
              <form onSubmit={chatMessageSubmit} className='flex items-center justify-between space-x-4.5'>
