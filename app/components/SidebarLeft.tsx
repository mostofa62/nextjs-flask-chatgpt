...
const SidebarLeft = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
...
.catch(function (error) {
  ...
  return (
    <aside
      ref={sidebar}
      className={`absolute left-0 top-0 z-9999 flex h-screen w-1/5 flex-col overflow-y-hidden bg-black duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
        sidebarOpen ? 'translate-x-0' : '-translate-x-full'
      }`}
    >
      {/* <!-- SIDEBAR HEADER --> */}
      <div className="flex items-center justify-between gap-2 px-6 py-1 lg:py-6.5">
...
<li key={1}>
  <Link
    ...
    className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
      pathname.substring(1,pathname.length)=='admin' && 'bg-graydark dark:bg-meta-4'
    }`}
  >
...
<li key={4}>
  <Link
    href={'/admin/users'}
    className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
      pathname.slice(-5) =='users' && 'bg-graydark dark:bg-meta-4'
    }`}
  >
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
...
<li key={5}>
  <Link
    href={'/admin/category'}
    className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
      pathname.slice(-8) =='category' && 'bg-graydark dark:bg-meta-4'
    }`}
  >
    <svg
                          className="fill-current"
...
<li key={6}>
  <Link
    href={'/admin/prompt'}
    className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
      pathname.slice(-7) =='prompt' && 'bg-graydark dark:bg-meta-4'
    }`}
  >
    <svg className="text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="18" height="20" fill="none" viewBox="0 0 18 20">
...
{authCtx.isLoggedIn &&
(
<div className='absolute bottom-4 flex flex-col w-10/12'>
<li key={1}>
  <Link
